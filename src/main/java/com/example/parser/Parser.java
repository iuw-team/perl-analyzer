...
import static com.example.parser.Token.Type;

//what changed: userFunctions
public class Parser {
private Tokenizer tokenStream;
private List<Statement> statements;
private List<Statement> userFunctions;
...
public List<Statement> getUserFunctions() {
...
private boolean isVarEnum(){
      tokenStream.freeze();
      boolean isEnumeration = isVariable() && isComa() && isVarEnum();
      if(!isEnumeration){
	    tokenStream.refresh();
	    isEnumeration = isVariable();
      }
      if(isEnumeration)
	    tokenStream.boost();
      else
...
private boolean isAssignable() {
      tokenStream.freeze();
      boolean result = isExpression() || isVarGroup();
      if(result)
	    tokenStream.boost();
      else
	    tokenStream.release();
...
private boolean isTableInitialization() {
      ...
      if (!isHeader) {
	    tokenStream.refresh();
	    isHeader = isBracket(OpenCircle) && isTableEntries() && isBracket(ClosedCircle);
      }
      if (isHeader)
	    tokenStream.boost();
      else
...
private boolean isDeclaration() {
      tokenStream.freeze();
      boolean result = isStorageClass() && isAssignable() && tokenStream.hasNext();
      if (result) {
	    tokenStream.freeze();
	    Token token = tokenStream.nextToken();
...
private boolean isExpValue() {
      ...
      if (!result) {
	    var types = List.of(Type.Number, Type.StringInner, Type.StringPlain);
	    for (Type type : types) {
		  tokenStream.refresh();
		  result = isEqualType(type);
		  if (result)
...
private boolean isFlowKeyword() {
      ...
      boolean isFlow = keyValue.equals("last") ||
			   ...
			   keyValue.equals("return") && isFuncParams();
...
private boolean isFuncDeclaration() {
      boolean result = isVCondBodyStatement(Statement.Type.Function, o -> isKeyword("sub") && isFloatWord());
      if (freeStatements != null)
	    userFunctions.addAll(freeStatements);
      return result;
}
